...
from datajoint.diagram import Diagram, _get_tier
from datajoint.settings import config
from datajoint.errors import DataJointError
from datajoint.jobs import JobTable
from datajoint.external import ExternalMapping
from datajoint.heading import Heading
from datajoint.utils import user_choice, to_camel_case
from datajoint.user_tables import Part, Computed, Imported, Manual, Lookup
from datajoint.table import lookup_class_name, Log, FreeTable
import types
...
class Schema:
    ...
    def drop(self, force=False, *, user_choice_fn = user_choice):
        """
        Drop the associated schema if it exists

        :param force   Positional param, if true, safemode is ignored and no user
                       dialog is done.
        ...
        if not self.exists:
            logger.info("Schema named `{database}` does not exist. Doing nothing.".format(database=self.database))
              ...
              force or
              user_choice_fn("Proceed to delete entire schema `%s`?" % self.database, default='no') == 'yes'):
